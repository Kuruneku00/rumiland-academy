...
export const RegistrationAttendancePage: React.FC = () => {
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      ...
      second_due_date: '',
      notes: ''
    }); setRegError(''); setShowRegDialog(true); setClasses([]); }}>پیش‌ثبت نام جدید</Button>
      ...
      <Card padding="0" style={{ marginBottom: '1.5rem' }}>
        ...
        <Table columns={regColumns} data={regRows} rowKey={(r) => r.registration.id} isLoading={loading}
          emptyState={<EmptyState title="هیچ ثبت‌نامی انجام نشده است" description="برای ثبت‌نام دانشجوی جدید کلیک کنید" action={<Button onClick={() => { setRegForm({
      student_id: '',
      course_id: '',
      class_id: '',
      registration_fee: 0,
